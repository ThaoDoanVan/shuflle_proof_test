...
static AGGREGATION_SIZES: [usize; 1] = [128];
    ...
    c.bench_function_over_inputs(
        ...
        move |b, &&m| {
            ...
            b.iter(|| {
                ...
                let result =proof.verify_multiple(&bp_gens, &pc_gens, &mut transcript, &value_commitments, n);
                assert!(result.is_ok(), "check failed ???????????");
